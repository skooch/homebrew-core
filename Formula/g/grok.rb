class Grok < Formula
  ...
  def install
    ...
    if DevelopmentTools.clang_build_version >= 1403
      inreplace "Makefile", "-O2", "-O2 -Wno-implicit-function-declaration"
    end

    if OS.linux?
      ENV.append "EXTRA_CFLAGS", "-I#{Formula["libtirpc"].opt_include}/tirpc"
      ENV.append "EXTRA_LDFLAGS", "-L#{Formula["libtirpc"].opt_lib} -ltirpc -ltokyocabinet -levent"

      ENV["GPERF"] = Formula["gperf"].opt_bin/"gperf"

      inreplace "conf.tab.c", "int yylineno;", "extern int yylineno;"
    end
